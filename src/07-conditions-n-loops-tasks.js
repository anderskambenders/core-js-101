...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.left >= rect1.left + rect1.width
...
function getDigitalRoot(num) {
  const sum = num.toString().split('').reduce((s, c) => +s + +c, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    let openIndex = open.indexOf(arr[i]);
...
function getCommonDirectoryPath(pathes) {
  const arr = pathes.map((val) => val.split('/'));
  const res = [];
  for (let i = 0; i < arr[0].length; i += 1) {
    const cur = arr[0][i];
    if (arr.every((el) => el[i] === cur)) res.push(arr[0][i]);
  }
...
function getMatrixProduct(m1, m2) {
  const resM = [];
  for (let i = 0; i < m1.length; i += 1) resM[i] = [];
...
function evaluateTicTacToePosition(pos) {
  for (let i = 0; i < pos.length; i += 1) {
    for (let j = 0; j < pos.length; j += 1) {
      if (pos[0][j] && pos[0][j] === pos[1][j]
        && pos[0][j] === pos[2][j]
      ) return pos[0][j];
    }
    if (pos[i][0] && pos[i][0] === pos[i][1]
      && pos[i][0] === pos[i][2]
    ) return pos[i][0];
  }
